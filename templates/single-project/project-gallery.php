						...
						<ul class="thumbnail-nav">
							<?php
								function addImageThumbnail($attachment_id, $zoomed_image_size){
									...
									$image       	= wp_get_attachment_image( $attachment_id, 'shop_thumbnail', false, array('data-caption' => $caption) );
									...
									echo apply_filters( 'woocommerce_single_product_image_thumbnail_html', sprintf( '
										<li>
											<div class="thumb" data-hq="%s" data-w="%s" data-h="%s" data-med="%s" data-medw="%s" data-medh="%s" data-caption="%s">%s</div>
										</li>',
										$hq[0], $hq[1], $hq[2], $med[0], $med[1], $med[2], $caption, $image ), $attachment_id, $post->ID );
								}
