 ...
 *
 * @package WordPress
 * @subpackage mz-project-archive
 * @author Mike iLL based on work by Matty
 * @since 1.0.0
 */
...
final class Projects {
	...
	public function post_type_names () {
		$this->singular_name 	= apply_filters( 'projects_post_type_singular_name', _x( 'Project', 'post type singular name', 'mz-project-archive' ) );
		$this->plural_name 		= apply_filters( 'projects_post_type_plural_name', _x( 'Projects', 'post type general name', 'mz-project-archive' ) );
	} // End post_type_names()
	...
	public function register_post_type () {
		...
		$labels = array(
			...
			'add_new' 				=> _x( 'Add New', $this->post_type, 'mz-project-archive' ),
			'add_new_item' 			=> sprintf( __( 'Add New %s', 'mz-project-archive' ), $this->singular_name ),
			'edit_item' 			=> sprintf( __( 'Edit %s', 'mz-project-archive' ), $this->singular_name ),
			'new_item' 				=> sprintf( __( 'New %s', 'mz-project-archive' ), $this->singular_name ),
			'all_items' 			=> sprintf( _x( 'All %s', $this->post_type, 'mz-project-archive' ), $this->plural_name ),
			'view_item' 			=> sprintf( __( 'View %s', 'mz-project-archive' ), $this->singular_name ),
			'search_items' 			=> sprintf( __( 'Search %a', 'mz-project-archive' ), $this->plural_name ),
			'not_found' 			=> sprintf( __( 'No %s Found', 'mz-project-archive' ), $this->plural_name ),
			'not_found_in_trash' 	=> sprintf( __( 'No %s Found In Trash', 'mz-project-archive' ), $this->plural_name ),
			'parent_item_colon' 	=> '',
		...
		$args = array(
			...
			'show_ui' 				=> true,
			'show_in_menu' 			=> true,
			// Following is what WC category is called: 'project-category'
    		// 'taxonomies'          => array( 'projects', 'project-category'),
    		// We are using 'project-category' so we could transfer all with a simple sql statement:
    		// UPDATE wp_term_taxonomy SET taxonomy='project-category' WHERE taxonomy='project-category';
    		// source: https://lukodp.wordpress.com/2013/04/18/transferring-categories-to-custom-taxonomy/
			'query_var' 			=> true,
	...
	public function load_localisation () {
		load_plugin_textdomain( 'mz-project-archive', false, dirname( plugin_basename( $this->file ) ) . '/lang/' );
