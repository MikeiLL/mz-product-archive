...
if ( ! function_exists( 'projects_page_title' ) ) {
	...
	function projects_page_title() {
		...
		if ( is_search() ) {
			$page_title = sprintf( __( 'Search Results: &ldquo;%s&rdquo;', 'projects-by-mzoo' ), get_search_query() );

			if ( get_query_var( 'paged' ) )
				$page_title .= sprintf( __( '&nbsp;&ndash; Page %s', 'projects-by-mzoo' ), get_query_var( 'paged' ) );
...
if ( ! function_exists( 'projects_taxonomy_category_list' ) ) {

	/**
	 * Show list of categories on taxonomy archives top level page
	 *
	 * Hooked into projects_archive_categories
	 *
	 * @access public
	 * @subpackage	Archives
	 * @return void
	 */
	function projects_taxonomy_category_list() {
		$home_page_id = get_option( 'projects-pages-fields' )['projects_page_id'];
		if (in_array(get_queried_object_id(), array($home_page_id, 0))) {
...
if ( ! function_exists( 'projects_template_get_breadcrumbs' ) ) {

	/**
	 * Display the project breadcrumbs.
	 *
	 * Hooked into projects_loop_item
	 *
	 * @access public
	 ...
	 * @return void
	 */
	function projects_template_get_breadcrumbs() {
		projects_get_template_part( 'content', 'global-breadcrumb');
	}
}
...
if ( ! function_exists( 'projects_template_single_feature' ) ) {
	 ...
	 *
	 * Hooked into projects_before_single_project_description
	 *
	 * @access public
	 * @subpackage	Project
	 * @return void
...
if ( ! function_exists( 'projects_template_single_description' ) ) {
	 ...
	 *
	 * Hooked into projects_single_project_description
	 *
	 * @access public
	 * @subpackage	Project
	 * @return void
	...
	function projects_output_product() {
		if ( class_exists( 'WooCommerce' ) && is_singular( 'project' ) ) {
			...
			if ( isset( $product_id ) && '' != $product_id && '0' != $product_id ) {
				...
				echo '<h3>' . apply_filters( 'projects_meta_woocommerce_title', __( 'Purchase', 'projects-by-mzoo' ) ) . '</h3>';
